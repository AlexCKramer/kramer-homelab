...
1. **Provider Selection (Authentik):** Selected over Authelia due to its robust, enterprise-grade feature set, built-in application outposts, and highly customizable authentication flows.
2. **Authentication Policy (No 2FA):** During the deployment, I successfully configured and tested 2FA implementation. However, I elected to disable it for daily operations. The lab services are not exposed to the public internet; they exist entirely within a local network protected by a UniFi Dream Router handling IDS/IPS.
